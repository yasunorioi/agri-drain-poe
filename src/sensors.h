// sensors.h — DFRobot SEN0575 tipping-bucket gauge over Modbus RTU,
// repurposed as a drainage (排液) volume sensor. Same wire protocol and
// register layout as agri-rain-poe / the agri-relay OGMS reference; the
// only semantic change is tips × ml_per_tip → cumulative drainage mL.
...
extern uint32_t g_cum_rain_raw;    // 0.0001 mm units (sensor-internal, diagnostic)
...
// Sensor-internal cumulative depth (its own bucket geometry). Diagnostic
// only — drainage volume must go through ml_per_tip calibration instead.
inline float sensorMm() {
  return g_cum_rain_raw / 10000.0f;
}

// Cumulative drainage volume. The SEN0575 board keeps the tip counter on
// its own MCU, so this survives an ESP32 reboot (resets only when the
// sensor itself loses power). The central computes per-cycle deltas.
inline float drainageMl() {
  return g_raw_tips * g_cfg.ml_per_tip;
}
